...
public class PacoteService {
			...
			for (Pacote pacote : lista) {
				if (pacote.getEmpresa().isAtivo()) {
					if (!pacote.isLixeira()) {
						...
						dto.setLixeira(pacote.isLixeira());
						listaDTO.add(dto);
					}
				}
			}
	...
	public List<PacoteDTO> getAllByEmpresa(String nome) throws EmpresaDesativadaException, EmpresaNotFoundException {
		List<Pacote> lista = repository.findAllByEmpresaNomeFantasia(nome);
		if (lista.isEmpty()) {
	...
	public void update(Integer id, PacoteDTO Dto)
			throws PacoteNotFoundException, CentroDistribuicaoNotFoundException, CentroDistribuicaoDesativadoException {
		Optional<Pacote> optional = repository.findById(id);
		...
		if (Dto.getCd().getNomeFantasia() != null && !Dto.getCd().getNomeFantasia().equals("")) {
			cd = cdService.getCdByDTO(cdService.getOneByNomeFantasia(Dto.getCd().getNomeFantasia()));
		}
		...
		if (Dto.getCd() != null && cd != null) {
			pacote.setCd(cd);
		}
		repository.save(pacote);
	}

	public void deactivate(Integer id) throws PacoteNotFoundException, PacoteDesativadoException {
		Optional<Pacote> pacote = repository.findById(id);
		if (pacote.isEmpty()) {
			throw new PacoteNotFoundException();
		}
		if (pacote.get().isLixeira()) {
			throw new PacoteDesativadoException();
		}
		pacote.get().setLixeira(true);
		repository.save(pacote.get());
	}

	public void activate(Integer id) throws PacoteNotFoundException, PacoteAtivoException {
		Optional<Pacote> pacote = repository.findById(id);
		if (pacote.isEmpty()) {
			throw new PacoteNotFoundException();
		}
		if (!pacote.get().isLixeira()) {
			throw new PacoteAtivoException();
		}
		pacote.get().setLixeira(false);
		repository.save(pacote.get());
	}
	...
	public void delete(Integer id) throws PacoteNotFoundException, PacoteAtivoException {
		Optional<Pacote> pacote = repository.findById(id);
		if (pacote.isEmpty()) {
			throw new PacoteNotFoundException();
		}
